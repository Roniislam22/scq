...
(function () {
  ...
  try {
    ...
    (function interceptSettingsAndNetwork() {
      function isDemoPathNow() { try { return location.pathname.indexOf('/en/demo-trade')===0; } catch(e){ return false; } }
      function isLivePathNow() { try { const p=location.pathname; return p.indexOf('/en/trade')===0 && p.indexOf('/en/demo-trade')!==0; } catch(e){ return false; } }
        ...
        if (OrigWS && !OrigWS.__qmPatched) {
          window.WebSocket = function(url, protocols){
            ...
            ws.addEventListener('message', function(ev){
              try {
                if (typeof ev.data==='string' && (ev.data.indexOf('isDemo')!==-1 || ev.data.indexOf('DEMO')!==-1)) {
                  // let app handle, but also try to flip incoming isDemo so UI stays live-like
                  // can't mutate ev, but we can dispatch a patched clone via overwriting ws.onmessage handler chaining
                }
              } catch(e){}
            });
            return ws;
    ...
    (function hookWebGLEarly() {
      ...
      function shouldBlockDEMOWatermark(count) {
        try { if (location.pathname.indexOf('/en/demo-trade') !== 0) return false; } catch(e){ return false; }
  ...
  const DEMO_WATERMARK_STYLE_ID = 'demo-watermark-remover';
  ...
  // Supported domains
  const SUPPORTED_DOMAINS = ['qxbroker.com', 'market-qx.info', 'market-qx.pro', 'market-qx.trade', 'market-qtx.trade'];
  const DEMO_PATH = '/en/demo-trade';
  const TRADE_PATH = '/en/trade';

  // (blocks are handled inside hookWebGLEarly; no duplicate needed)

  // ============================================================
  // DEMO ↔ LIVE Chart Swap (user request)
  // When on DEMO -> show as Live, when on LIVE -> show as Demo
  // ============================================================
  const CHART_SWAP_ENABLED = true;
  const isDemoPath = () => location.pathname.startsWith(DEMO_PATH);
  const isLivePath = () => location.pathname.startsWith(TRADE_PATH) && !isDemoPath();
  ...
  // ============================================================
  // Redirect demo-trade to trade (DISABLED when chart swap enabled)
  // ============================================================
  function redirectDemoToTrade() {
    if (CHART_SWAP_ENABLED) return; // swap mode: keep URL as-is to allow visual swap
  ...
  // ============================================================
  // DEMO ↔ LIVE Chart Swap — Watermark
  // In DEMO (/en/demo-trade): WebGL watermark already blocked in early hooks
  // In LIVE  (/en/trade):     inject DEMO overlay
  // ============================================================
  function installDemoWatermarkRemover() {
    if (window.__qmDemoWatermarkRemoverInstalled) return;
    window.__qmDemoWatermarkRemoverInstalled = true;

    // DEMO path: already handled by early document_start hooks (fillText mark + WebGL draw 1710/6 block)
    // Keep only a lightweight repaint helper here if needed — no extra prototype patching (prevents freeze)
    if (CHART_SWAP_ENABLED && isDemoPath()) {
      setTimeout(() => {
        const h = document.querySelector('#graph, .chart-container');
        if (h) { h.style.display='none'; void h.offsetHeight; h.style.display=''; window.dispatchEvent(new Event('resize')); }
      }, 900);
      return;
    }

    // ---- LIVE path: INJECT DEMO watermark so LIVE looks like DEMO ----
    if (CHART_SWAP_ENABLED && isLivePath()) {
      const overlayStyle = document.createElement('style');
      overlayStyle.id = DEMO_WATERMARK_STYLE_ID;
      overlayStyle.textContent = `
        ...
        .chart-container, #graph, .trading-chart__wrapper .chart, [class*="chart-container"] {
          position: relative !important;
        }
      `;
      const addOverlayStyle = () => document.head && document.head.appendChild(overlayStyle);
      if (document.head) addOverlayStyle();
      else document.addEventListener('DOMContentLoaded', addOverlayStyle, { once: true });

      function ensureOverlay() {
        const container = document.querySelector('.chart-container, #graph, .trading-chart__wrapper .chart, [class*="chart-container"]');
        if (!container) return false;
        const cs = getComputedStyle(container);
        if (cs.position === 'static') container.style.position = 'relative';
        if (container.querySelector('#qm-demo-overlay')) return true;
        const overlay = document.createElement('div');
        overlay.id = 'qm-demo-overlay';
      ...
      // Also paint DEMO via canvas on every frame for authenticity
      const originalFillText = CanvasRenderingContext2D.prototype.fillText;
      if (!CanvasRenderingContext2D.prototype.__qmDemoInjector) {
        CanvasRenderingContext2D.prototype.__qmDemoInjector = true;
        CanvasRenderingContext2D.prototype.fillText = function(text, x, y) {
          const res = originalFillText.apply(this, arguments);
          try {
            const canvas = this.canvas;
            if (!canvas) return res;
            // only for main chart canvas (large)
            if (canvas.width >= 500 && canvas.height >= 250 && canvas.classList.contains('layer')) {
              this.save();
              this.globalAlpha = 0.18;
              this.fillStyle = '#ffffff';
              this.font = '900 ' + Math.round(canvas.width / 9) + 'px "Segoe UI", Arial, sans-serif';
              this.textAlign = 'center';
              this.textBaseline = 'middle';
              // rotate -16deg around center
              this.translate(canvas.width / 2, canvas.height / 2);
              this.rotate(-16 * Math.PI / 180);
              this.fillText('DEMO', 0, 0);
              this.restore();
            }
          } catch {}
          return res;
      ...
      const overlayTimer = setInterval(() => {
        if (ensureOverlay()) { clearInterval(overlayTimer); }
        if (++overlayAttempts > 60) clearInterval(overlayTimer);
      }, 500);
      document.addEventListener('DOMContentLoaded', ensureOverlay, { once: true });
      window.addEventListener('load', ensureOverlay, { once: true });
      new MutationObserver(ensureOverlay).observe(document.documentElement, { childList: true, subtree: true });
      setTimeout(ensureOverlay, 800);
      return;
    }

    // Fallback: neither swap path matched, do nothing
  }

  installDemoWatermarkRemover();

  // (header swap is handled inside initMenuAccountDisplay — no duplicate observer needed)
  ...
  // ============================================================
  // Initialize Firebase & Start Extension
  // ============================================================
  ...
  function initializeFirebase() {
    ...
    function initMenuAccountDisplay() {
      ...
      // ---- Top header account badge — SWAPPED when chart swap enabled ----
      const USERMENU_SELECTORS = ".QE4Zb, .Xlyoi, .rymiA, [class*='usermenu']";
      const USERMENU_NAME_SELECTORS = ".v2KPX, .SfrTV, .lTzTl, [class*='infoName']";
      ...
      function processUsermenuForLive(usermenuEl) {
        try {
          ...
          // In swap mode: respect DEMO↔LIVE swap
          let liveText, color;
          if (CHART_SWAP_ENABLED) {
            if (isDemoPath()) { liveText = getUsermenuLiveText(); color = '#0faf59'; }
            else if (isLivePath()) { liveText = 'Demo'; color = ''; }
            else { liveText = getUsermenuLiveText(); color = '#0faf59'; }
          } else {
            liveText = getUsermenuLiveText(); color = '#0faf59';
          }

          for (const nameEl of nameEls) {
            if (nameEl.textContent === liveText && nameEl.style.color === (color ? 'rgb(15, 175, 89)' : '')) continue;
            // only swap if it looks like Demo/Live
            const t = nameEl.textContent.trim();
            if (t !== 'Demo' && t !== 'Live' && t !== 'Demo Account' && t !== 'Live Account') continue;
          ...
          console.error('Usermenu update error:', error);
        }
      }
      ...
      // throttle swap observer: RAF + debounce to avoid freeze from chart's high-frequency mutations
      let usermenuPending = false;
      const usermenuMo = new MutationObserver(() => {
        if (usermenuPending) return;
        usermenuPending = true;
        requestAnimationFrame(() => {
          usermenuPending = false;
          updateUsermenuToLive();
        });
      });
      usermenuMo.observe(document.documentElement, { childList: true, subtree: true, characterData: false, attributes: false });
      setInterval(updateUsermenuToLive, 1400);
      setTimeout(updateUsermenuToLive, 700);
    }
  ...
  // ============================================================
  // Tab Title Enforcer — swapped when chart swap enabled
  // DEMO (/en/demo-trade) -> "Live trading | Quotex"
  // LIVE (/en/trade)      -> "Demo trading | Quotex"
  // ============================================================
  (function () {
    if (window.extensionDisabled) return;

    function getDesiredTitle() {
      if (CHART_SWAP_ENABLED && isDemoPath()) return 'Live trading | Quotex';
      if (CHART_SWAP_ENABLED && isLivePath()) return 'Demo trading | Quotex';
      return 'Live trading | Quotex';
    }

    function enforceTitle() {
      const DESIRED_TITLE = getDesiredTitle();
    ...
    function observeTitle() {
      ...
      if (titleEl) {
        ...
        newTitle.textContent = getDesiredTitle();
